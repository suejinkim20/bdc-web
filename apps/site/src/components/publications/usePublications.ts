...
export type SortOption = 'most-recent' | 'least-recent' | 'title-az' | 'title-za';
...
export function usePublications(publications: Publication[]) {
  ...
  const filtered = useMemo(() => {
    ...
    if (filters.researchCommunity.length > 0) {
      result = result.filter(pub =>
        pub.researchCommunity?.some(rc => filters.researchCommunity.includes(rc))
      );
    }
...
const filterOptions = useMemo(() => {
  ...
  const forYearCounts = applySearch(publications.filter(pub => {
    const inCommunity = filters.researchCommunity.length === 0 ||
      pub.researchCommunity?.some(rc => filters.researchCommunity.includes(rc));
    const inArea = filters.researchArea.length === 0 ||
      pub.researchArea?.some(ra => filters.researchArea.includes(ra));
    const inOrg = filters.bdcContribution.length === 0 ||
      pub.bdcContribution?.some(oc => filters.bdcContribution.includes(oc));
  ...
  const forCommunityCounts = applySearch(publications.filter(pub => {
    const inYear = filters.year.length === 0 ||
      filters.year.includes(String(new Date(pub.date).getFullYear()));
    const inArea = filters.researchArea.length === 0 ||
      pub.researchArea?.some(ra => filters.researchArea.includes(ra));
    const inOrg = filters.bdcContribution.length === 0 ||
  ...
  const forAreaCounts = applySearch(publications.filter(pub => {
    const inYear = filters.year.length === 0 ||
      filters.year.includes(String(new Date(pub.date).getFullYear()));
    const inCommunity = filters.researchCommunity.length === 0 ||
      pub.researchCommunity?.some(rc => filters.researchCommunity.includes(rc));
  ...
  const forOrgCounts = applySearch(publications.filter(pub => {
    const inYear = filters.year.length === 0 ||
      filters.year.includes(String(new Date(pub.date).getFullYear()));
    const inCommunity = filters.researchCommunity.length === 0 ||
      pub.researchCommunity?.some(rc => filters.researchCommunity.includes(rc));
